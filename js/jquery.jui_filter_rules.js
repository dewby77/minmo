...
(function ($) {
		...
		getRules: function (rules_group_index, a_rules) {
			...
			for (r = 0; r < a_group_rules_len; r++) {
				...
				if (group_rule.tagName == 'LI') {
					...
					current_rule.condition.conditionOperator = filter_operator;
	...
	var create_rules_group_tools = function (container_id) {
		...
		//TODO: McCrea - never seen these options not disabled, not sure proper config, so do not display if not enabled
		if (disabled_html.length === 0) {
	...
	var create_filters_list = function (container_id) {
		...
		//TODO: McCrea - override in grid is perpetuating to here which is causing style issues, need new property for filter line style
		//f_html += '<div class="' + filterContainerClass + '">';
		f_html += '<div class="filter_container">';
