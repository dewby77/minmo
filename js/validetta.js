; (function ($) {
    ...
    Validetta.prototype = {
        ...
        events: function () {
            ...
            //McCrea - validetta is very event driven, create a custom event for it to work for non-form events
            $(this.form).on('validetta:clear', function () {
                $(self.form.querySelectorAll('.' + self.options.errorClass + ', .' + self.options.validClass))
                  .removeClass(self.options.errorClass + ' ' + self.options.validClass);
                return self.reset();
            });

            $(this.form).on('validetta:validate', function (e) {
                // fields to be controlled transferred to global variable
                FIELDS = this.querySelectorAll('[data-validetta]');
                return self.init(e);
            });
        ...
        init: function (e) {
            ...
            this.checkFields(e);
            //McCrea - adding our new custom event
            if (e.type !== 'submit' || e.type !== "validetta:validate") return; // if event type is not submit, break
